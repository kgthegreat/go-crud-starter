...
type Tmpl struct {
	*template.Template                  // root template
	...
	devel              string           // reload every time
...
func NewTmpl(dir, ext string, devel string) (tmpl *Tmpl, err error) {
...
func (t *Tmpl) Render(w http.ResponseWriter, name string, data interface{}) {
	...
	if t.devel == "local" {
		...
		// reload
		if modified == true {
			log.Println("Reloading since templates changed")
